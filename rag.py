...
def build_router_query_engine():
    ...
    # 建圖成本高（150+ 次 LLM 呼叫），建好後持久化，之後啟動直接載入
    if os.path.exists(GRAPH_PERSIST_DIR):
        ...
        kg_extractor = SchemaLLMPathExtractor(
            # 抽取用的 LLM 加大重試次數：NVIDIA endpoint 對請求量敏感（503），
            # 靠指數退避慢慢送完；模型與其他索引相同
            llm=llm.model_copy(update={"max_retries": 8}),
        ...
        if in_running_loop:
            with ThreadPoolExecutor(max_workers=1) as pool:
                graph_index = pool.submit(_build_graph_index).result()
        else:
            graph_index = _build_graph_index()
