...
連線、從 rag_indexes.py 取得四種索引，包成 QueryEngineTool 後交由
...
執行流程：
    ...
    2. 透過 rag_indexes 讀取 ./data 並建立四個索引
...
def build_router_query_engine():
    ...
    keyword_index = rag_indexes.build_keyword_index(documents, splitter, summary_llm)  # 抽關鍵字用便宜模型
