...
class RecordPathExtractor(TransformComponent):
    ...
    # 知識圖譜的節點與關係結構：
    #   - 旅次 節點代表一次旅行事件（如 03_台南_2025年1月），原文 chunk 都掛在它底下
    #   - 旅伴 節點代表同行者類型（如 1人（自己去）），是「按旅伴聚合」的查詢入口
    #   - 目的地 節點代表地點（如 台南），重訪同一地點時可跨旅次合併
    # 節點之間會用關係連起來，讓查詢可以沿著知識圖譜尋找答案：
    #   - (旅次)-[同行]->(旅伴)：表示這趟旅行和誰一起去
    #   - (旅次)-[造訪]->(目的地)：表示這趟旅行去了哪裡

    def __call__(self, nodes, **kwargs):
        # 逐個 chunk 產生三元組
        for node in nodes:
            # 建立旅次節點：名稱取自 metadata 的「旅遊紀錄」（即檔名），同名自動合併
...
def build_graph_index(documents, splitter, llm, embed_model):
    ...
    return PropertyGraphIndex.from_documents(
        # 每筆紀錄的 metadata 帶旅次與旅伴（規則抽取的資料來源）
